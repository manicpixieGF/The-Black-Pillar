...
- `replaceOrgLatex`: If `true` (default), converts Org-mode [[99 - Assets/docs/features/Latex|Latex]] fragments to Quartz-compatible LaTeX wrapped in `$` (for inline) and `$$` (for block equations).
